...
def upload_payment_proof_to_storage(
    filename: str,
    content_type: str,
    image_bytes: bytes,
) -> tuple[bool, str, str | None]:
    ...
    if config_error:
        return False, config_error, None
    if not image_bytes:
        return False, "The uploaded payment proof is empty.", None

    bucket = os.environ.get("SUPABASE_PAYMENT_PROOFS_BUCKET", "payment-proofs").strip() or "payment-proofs"
    object_path = f"orders/{filename}"
    supabase_url, _ = current_supabase_service_config()
    try:
        bucket_response = requests.post(
            f"{supabase_url}/storage/v1/bucket",
        ...
        return False, "Unable to prepare the payment proof storage bucket right now.", None
    ...
    try:
        ...
        return False, "Unable to upload payment proof to Supabase Storage right now.", None
    ...
    public_url = f"{supabase_url}/storage/v1/object/public/{bucket}/{object_path}"
    return True, "Payment proof uploaded successfully.", public_url
